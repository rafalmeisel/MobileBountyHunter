...
def run_check_android_manifest_allow_backup(application_package_system, application_package_name, file_name, xml_file_content):

    allow_backup_items_list = check_android_manifest_allow_backup(xml_file_content)
    
    if len(allow_backup_items_list) > 0:
        for allow_backup_item in allow_backup_items_list:
            report_issue(application_package_system, application_package_name, file_name, IssueSeverity.LOW, IssueStatus.VULNERABLE, "allowBackup", allow_backup_item)
    else:
        report_issue(application_package_system, application_package_name, file_name, IssueSeverity.INFORMATIVE, IssueStatus.NOT_FOUND, "allowBackup", "")